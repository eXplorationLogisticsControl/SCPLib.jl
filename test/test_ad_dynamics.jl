...
# function test_ad_dynamics()
    ...
    eom_aug_ad! = SCPLib.get_continuous_augmented_eom(eom!, params, nx)
    ...
    sol_aug    = solve(ODEProblem(eom_aug!, x0_aug, [0.0, tf], params), Vern8(), reltol=1e-14, abstol=1e-14)
    sol_aug_ad = solve(ODEProblem(eom_aug_ad!, x0_aug, [0.0, tf], params), Vern8(), reltol=1e-14, abstol=1e-14)

    @test maximum(abs.(sol.u[end] - sol_aug.u[end][1:6])) < 1e-13
    @test maximum(abs.(sol.u[end] - sol_aug_ad.u[end][1:6])) < 1e-13
    @show maximum(abs.(sol.u[end] - sol_aug.u[end][1:6]))
    @show maximum(abs.(sol.u[end] - sol_aug_ad.u[end][1:6]))

    Phi_A = reshape(sol_aug.u[end][nx+1:nx*(nx+1)], (nx,nx))'
    Phi_B = reshape(sol_aug.u[end][nx*(nx+1)+1:nx*(nx+1)+nx*nu], (nu,nx))'
    Phi_A_ad = reshape(sol_aug_ad.u[end][nx+1:nx*(nx+1)], (nx,nx))'
    Phi_B_ad = reshape(sol_aug_ad.u[end][nx*(nx+1)+1:nx*(nx+1)+nx*nu], (nu,nx))'
    @show maximum(abs.(Phi_A - Phi_A_ad)) #< 1e-12
    @show maximum(abs.(Phi_B - Phi_B_ad)) #< 1e-12
    ...
    lines!(Array(sol_aug_ad)[1,:], Array(sol_aug_ad)[2,:], Array(sol_aug_ad)[3,:], color=:red)
    display(fig)
# end
