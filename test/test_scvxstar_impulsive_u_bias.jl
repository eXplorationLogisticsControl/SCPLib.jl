"""Dev for impulsive problem with bias on control"""
...
function test_scvxstar_impulsive_u_bias(;verbosity::Int = 0, get_plot::Bool = false)
    ...
    u_bias_list = [zeros(nu, N), 5e-4 * ones(nu, N)]
    sols_opt_list = []
    solution_list = []
    ...
    if get_plot
        ...
        for (iprob, solution) in enumerate(solution_list)
            stem!(axu, times, solution.u[4,:], color=colors[iprob])
        end
        display(fig)
    end
end

test_scvxstar_impulsive_u_bias(verbosity = verbosity, get_plot=get_plot)