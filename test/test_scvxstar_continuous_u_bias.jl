"""Test problem with non-convex dynamics with bias on control"""
...
mutable struct ControlParams_continuous_u_bias
    μ::Float64
    u::Vector
    function ControlParams_continuous_u_bias(μ::Float64)
        new(μ, zeros(4))
    end
end

function test_scvxstar_continuous_u_bias(;verbosity::Int = 0, get_plot::Bool = false)
    μ = 1.215058560962404e-02
    DU = 389703     # km
    TU = 382981     # sec
    MU = 500.0      # kg
    VU = DU/TU      # km/s
    params = ControlParams_continuous_u_bias(μ)
    ...
    # solve without & with bias
    u_bias_list = [zeros(nu, N), 1e-2 * ones(nu, N)]
    sols_opt_list = []
    solution_list = []
    ...
    if get_plot
        ...
        colors = [:black, :red]
        for (iprob, sols_opt) in enumerate(sols_opt_list)
            for (isol, _sol) in enumerate(sols_opt)
                lines!(ax3d, Array(_sol)[1,:], Array(_sol)[2,:], Array(_sol)[3,:], color=colors[iprob])
                scatter!(ax3d, Array(_sol)[1,1], Array(_sol)[2,1], Array(_sol)[3,1], color=colors[iprob])
                scatter!(ax3d, Array(_sol)[1,end], Array(_sol)[2,end], Array(_sol)[3,end], color=colors[iprob])
            end
        end
        ...
        for (iprob, solution) in enumerate(solution_list)
            stem!(axu, times[1:end-1], solution.u[4,:], color=colors[iprob])
        end
        display(fig)
    end
    return
...
test_scvxstar_continuous_u_bias(;verbosity = verbosity, get_plot = get_plot)