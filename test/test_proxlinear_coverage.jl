"""Exercise the prox-linear branches that the physical test problem never reaches"""
...
proxlinear_coverage_g_noncvx(cache, x, u) = [x[1,2]^2 - 0.25]
proxlinear_coverage_h_noncvx(cache, x, u) = [x[1,2]^2 - 1.0]


"""Single-state integrator `ẋ = u` steered from 0 to 1"""
function make_proxlinear_coverage_problem(optimizer;
...
function test_proxlinear_verbose_run_to_maxiter()
    prob, x_ref, u_ref = make_proxlinear_coverage_problem(Clarabel.Optimizer;
        ng = 1, g_noncvx = proxlinear_coverage_g_noncvx,
        nh = 1, h_noncvx = proxlinear_coverage_h_noncvx)
    algo = SCPLib.ProxLinear(1e2, 1e0; proximal_u = true)
...
function test_proxlinear_feasible_at_maxiter()
    prob, x_ref, u_ref = make_proxlinear_coverage_problem(Clarabel.Optimizer)
...
function test_proxlinear_warmstart()
    prob, x_ref, u_ref = make_proxlinear_coverage_problem(SCS.Optimizer)
    algo = SCPLib.ProxLinear(1e2, 1e0)
...
function test_proxlinear_infeasible_subproblem()
...
test_proxlinear_verbose_run_to_maxiter()
test_proxlinear_feasible_at_maxiter()
test_proxlinear_warmstart()
test_proxlinear_infeasible_subproblem()
