...
# -------------------- setup problem -------------------- #
struct ControlParams_fixedtrw_impulsive_dynamics_only
    μ::Float64
end
...
function test_scvxstar_impulsive_dynamics_only(;verbosity::Int = 0, get_plot::Bool = false)
    ...
    tol_opt = 1e-6
    tol_feas = 1e-8
    solution = SCPLib.solve!(algo, prob, x_ref, u_ref; verbosity = verbosity, maxiter = 100, tol_opt = tol_opt, tol_feas = tol_feas)

    # propagate solution
    sols_opt, g_dynamics_opt = SCPLib.get_trajectory(prob, solution.x, solution.u)
    @test maximum(abs.(g_dynamics_opt)) <= tol_feas
    @test solution.status == :Optimal
    @test solution.info[:J0][end] ≈ 0.21345347660338765 atol=1e-8
