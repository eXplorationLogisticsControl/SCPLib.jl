...
function stack_flatten_variables(prob::ContinuousProblem, x, u)
    x_len = prob.shooting_method == :forwardbackward ? prob.nx * 2 : prob.nx * prob.N
    Δz = [reshape(x, x_len);
...
function unpack_flattened_variables(prob::ContinuousProblem, z)
    ...
    x_len = prob.nx * x_cols
    u_len = prob.nu * (prob.N-1)
    x = reshape(z[1:x_len], prob.nx, x_cols)
    u = reshape(z[x_len + 1:x_len + u_len], prob.nu, prob.N-1)
    return x, u
end
